...
@foreach($bahan as $b)
<div class="modal fade" id="editBahan{{ $b->id }}" tabindex="-1">
...
document.getElementById('namaBahan').addEventListener('input', function () {
    ...
    if (!nama) return document.getElementById('kodeBahan').value = '';

    let singkatan = nama.split(' ')
        .filter(w => w.length)
        .map(w => w[0])
        .join('')
        .toUpperCase();
    ...
    document.getElementById('kodeBahan').value =
        singkatan + '-' + Math.floor(1000 + Math.random() * 9000);
});
</script>
@endsection
